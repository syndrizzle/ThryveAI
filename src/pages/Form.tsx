...
import { Drop, Timer, Waves, Moon, ChatCircleDots } from '@phosphor-icons/react';
    ...
    const handleStartCall = async () => {
        ...
        try {
            const { data: profile, error: profileError } = await supabase
                .from('user_profiles')
                .select('phone_number, subscription_plan')
                .eq('user_id', user.id)
                .single();
            ...
            if (!profile?.phone_number) {
                console.warn("Phone number missing, redirecting to onboarding.");
                navigate('/onboarding'); 
                setLoading(false);
                return;
            }
              ...
              <div className="flex-1 flex flex-col items-center justify-center max-w-5xl mx-auto w-full space-y-12 py-8">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full sm:w-auto relative group p-[1px] rounded-full overflow-hidden animate-border [background:linear-gradient(45deg,transparent,theme(colors.neutral.900)_40%,transparent)_padding-box,conic-gradient(from_var(--border-angle),theme(colors.neutral.600/.3)_20%,theme(colors.white)_40%,theme(colors.neutral.600/.3))_border-box]"
                  >
                    <div className="relative px-8 py-4 rounded-full bg-neutral-950/90 backdrop-blur-sm text-white font-medium group-hover:bg-neutral-900/90 transition-colors">
                      {loading ? (
                  ...
                  <motion.button
                    ...
                    className="w-full sm:w-auto relative group p-[1px] rounded-full overflow-hidden animate-border [background:linear-gradient(45deg,transparent,theme(colors.neutral.900)_40%,transparent)_padding-box,conic-gradient(from_var(--border-angle),theme(colors.neutral.600/.3)_20%,theme(colors.white)_40%,theme(colors.neutral.600/.3))_border-box]"
                  >
                    <div className="relative px-8 py-4 rounded-full bg-neutral-950/90 backdrop-blur-sm text-white font-medium group-hover:bg-neutral-900/90 transition-colors flex items-center gap-3">
